...
//	private static final String QUERY_FILE = "data/sparql/query.rq";
	...
	public static void findByMetadata(MetadataSearchParams param) throws IOException {
		String whereQueryPart = createWhereQueryPart(param);
		ResultSetRewindable results = select(whereQueryPart);
		
		ResultSetFormatter.out(System.out, results);
		results.reset();
		List<String> idsDocuments = getDocumentIds(results);
		System.out.println(idsDocuments);
	}
	
	public static void findByMetadata(List<MetadataSearchParams> params) throws IOException {
		findByCombinedMetadata(params);
	}
	
	public static void findByCombinedMetadata(List<MetadataSearchParams> params) throws IOException {
